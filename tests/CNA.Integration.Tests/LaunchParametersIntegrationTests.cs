...
public class LaunchParametersIntegrationTests(ITestOutputHelper output, NativeGameFixture fixture)
{
    [NativeFact]
    ...
    public void LaunchParameters_AgreeWithTheKeyedAccessors()
    {
        fixture.InsideAFrame(game =>
        {
            game.AddLaunchParameter("agree-alpha", "one");
            game.AddLaunchParameter("agree-beta", "two");

            foreach ((string key, string value) in game.LaunchParameters)
            {
    ...
    {
        // Keys are unique to this test. The game is shared across the whole assembly, so a key
        // another test also writes would make this assert on that test's value -- which is exactly
        // what happened with a bare "alpha".
        string[] added = ["enum-zulu", "enum-alpha", "enum-mike", "enum-bravo"];
