...
/// <summary>
/// One capability of the active renderer. A duplicate of
/// <see cref="CNA.Graphics.GraphicsCapability"/> in this namespace, the same way every other CNA
/// enum is mirrored here, so a compat game never has to name a <c>CNA.Graphics</c> type. The
/// numeric values are identical and the two convert by cast.
///
...
public enum GraphicsCapability : uint
{
    ...
    /// <c>new Effect(GraphicsDevice, byte[])</c> and <c>Load&lt;Effect&gt;</c> need for a
