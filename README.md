...
`RenderTarget2D`, `ContentManager`, `Keyboard`, `Mouse`, `GamePad`) has its
managed side built and compiles, but does **not** yet work end to end,
because it depends on a stable C ABI in
...
and `dotnet test` passes all 54 unit tests. Running `samples/HelloGame`
