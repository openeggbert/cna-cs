...
| Native integration | 207 tests in Debug and Release against **five** ABI 0.21.0 renderers on Linux x64. VULKAN, SDL_RENDERER, SOFTWARE and HEADLESS: **207/207 in both configurations**. OPENGLES3: **203/207**, the four being the one EasyGL clear defect wearing three costumes plus its own minimal statement. OPENGL33 cannot be captured at all -- it segfaults on the second `Game` -- and that is a blocker-table row rather than an unknown. **Zero silent capability branches remain on any renderer**; 31 absent branches assert on SDL_RENDERER alone |
...
| Compiled-content survey | XNA 4.0 sample collection (4,369 assets, still growing under this measurement), `--load`: **4,263 attempted, 4,187 loaded, 0 needing a reader this binding lacks, 0 malformed, and 0 refused by the native loader**, 35 failing, 41 needing a game's own assembly. **The native refusals went 33 -> 0** when upstream taught the C content loader the non-`Color` surface formats. The 35 failures have not moved as the corpus grew, and every one is classified with none a binding defect: 29 Xbox 360 compiled effects (bytecode with no XNA Direct3D 9 Effect Framework header), 4 XMA2 sounds (`formatTag=358`), 2 models whose `.cnj` effect is absent from that build output. `cna-samples` (938 assets): 919 attempted, **912 loaded, 0 failing, 0 refused** |
...
- D2. **First vertical slice done.** `CNA.Content.Cnb.CnbDocument` opens a `.cnb`, reports its
  ...
  **Fifth slice done: sprite fonts.** `CnbSpriteFont` decodes the glyph table and metrics without a
  device, and `CnbSpriteFontLoader` uploads the atlas and builds a real
  `CNA.Graphics.SpriteFont` -- so a `.cnb` file becomes something a game can draw with, the way the
  texture slice made one into a `Texture2D`. Ten routes.

  Two things the file says that a value alone cannot: a font declaring **no** default character
  reports null rather than `'\0'`, because CNA carries the presence flag separately and collapsing
...
- E2. **Premise confirmed, and partly actionable.** The renderer removal has merged: sokol,
  ...
  **The configure block is gone** and two more renderers are built. `SDL_RENDERER` is the one that
  ...
  | --- | --- | --- | --- | --- | --- |
  | OPENGLES3 (EasyGL, GLES 3.2) | 1 (`MultiSampleAntiAliasing`) | version 2 | **203/207** -- the 4 are the clear defect | pass | **no** -- the EasyGL defect |
  | OPENGL33 (EasyGL, GL 4.6 core) | same | version 2 | **cannot be captured** -- segfaults on the second `Game` | pass | **no** -- identically |
  ...
  **`OPENGL33` is the one that narrowed the clear defect.** It is the *same* EasyGL renderer driving
  ...
  in code both profiles share rather than anything about GLES.

  **The SIGSEGV both GL profiles used to show has been split into two causes, and one of them was
  ours.** The `OPENGLES3` abort was this repository's test hard-coding `#version 330 core` and handing
  desktop GLSL to an ES context, which EasyGL answers by aborting inside `cna_shader_effect_create`
  rather than reporting an invalid source. Isolating it took running with detailed logging until the
  stop point proved stable, reproducing it from the class alone, then from one test alone, then
  instrumenting that test until the crash sat inside the create call rather than in the reflection
  the test is named for. The test now asks `ShadingDialect` the way its sibling always did, and
  `OPENGLES3` completes the suite. `OPENGL33` is a genuinely separate upstream fault: it segfaults on
  the **second** `Game` create/destroy cycle -- one succeeds, two do not, and 200 cycles are clean on
  the other four renderers -- so the full suite cannot be captured on it. Both are blocker-table rows
  now, with the reproducers bounded rather than described.
...
- **Half closed.** `VideoPlayer.GetTexture` needed "stable frame-slot identity *or* an explicit
  validity generation". The second exists as of this ABI and is bound:
  `cna_video_player_get_frame_ext` reports a monotonic generation that `Stop` and a change of video
  both leave alone, so inequality means the pixels changed. `CnaVideoPlayerExtensions.GetCnaFrame`
  exposes it, outside the strict contract because XNA has no such notion. What is still missing is
  the first half -- XNA's two stable `Texture2D` frame buffers -- since the alias still expires on
  the next player call. Whether the generation *advances* needs a legally redistributable video
  fixture and is `FIXTURE_BLOCKED`; everything reachable without one is asserted.
