...
| `VideoPlayer.GetTexture()` identity and lifetime -- **half closed** | The row asked for stable player-owned frame slots **or** a stable opaque identity plus an explicit validity generation. **The second exists and is bound.** `cna_video_player_get_frame_ext` reports a generation that counts decoded frames, is monotonic for the player's whole life, and is restarted by neither `Stop` nor a change of video -- so inequality means the pixels changed. What is still absent is the first half: the texture is borrowed on exactly the old terms and expires on the next call to the player, so XNA's two stable `Texture2D` frame buffers still cannot be reproduced. | Before `Play`, the strict facade throws `InvalidOperationException`. With a frame it exposes at most one non-owning transient `Texture2D` facade, invalidated before every subsequent player call, and `CnaVideoPlayerExtensions.GetCnaFrame` now reports the generation alongside it -- outside the strict contract, because XNA has no such notion. Whether the generation *advances* needs a legally redistributable video fixture and is unmeasured here; everything reachable without one is asserted, including the negative presentation time that distinguishes "no frame" from "a frame at t=0". | Expose stable player-owned frame slots, so a caller can hold a frame across a call the way XNA lets it. The generation half of this row is done. | One live borrowed facade with deterministic invalidation, plus the generation for callers who only need to know whether the frame moved. Never cache a duplicate owning wrapper or claim XNA-stable identity. |
...
| A virtual call into a `GraphicsDevice` that has begun destruction | Not a route: `GraphicsResource::Dispose` calls `graphicsDevice_->OnResourceDestroyed(name_, tag_)` at `modules/graphics/src/Xna/GraphicsResource.cpp:96`, and UBSan reports the target's vptr is already `System::Object` -- the derived part of the device's destructor has run. Found by building CNA with `-fsanitize=undefined` (`cnanext/build-ubsan`, HEADLESS, ABI 0.21.0), which the upstream configure block had made impossible until this generation. Reproduced by `SpriteFont_LoadsAnAuthoredCompressedAtlasThroughContent`, twice per run. **An ASan build (`cnanext/build-asan`) reports a heap-use-after-free from the same test** -- an 8-byte read, which is the size of a vptr. Two independent sanitizers, one reproduction, one story: the device's memory is freed and its vptr is then read to dispatch a virtual call. | Nothing observable in the ordinary build: the same suite is 207/207 with and without instrumentation, which is exactly why it needed a sanitizer to see. **Not caused by this binding's content-manager ownership change** -- both sanitizers were re-run with that disposal removed and report identically, which was checked before reporting either. | Order the teardown so a resource cannot reach its device after the device's derived destructor has run -- either by clearing `graphicsDevice_` on the device's way down, or by disposing registered resources before it. | None, and none is possible from managed code: the ordering is entirely inside CNA's C++ destructors. **Reproducing needs `UBSAN_OPTIONS=log_path=...`** -- `vstest` swallows the test host's native stderr, so a run without it reports zero and means nothing. |
| Shutdown on `SIGTERM` | Not a route: a game terminated by signal crashes during teardown. **Reproduced on the current build**, which is what this row is for -- the report had sat as "open until reproduced or disproved" long enough to be worth settling, and it is real. Measured over three runs per renderer with `scripts/Reproduce-SigtermShutdown.sh`, which sends `SIGTERM` to a template that has been drawing for ten seconds: `SOFTWARE` prints `pure virtual method called` then `terminate called without an active exception` and aborts, **3 of 3**; `OPENGLES3` segfaults, **3 of 3**, a different signature reached the same way; `HEADLESS` shows the `SOFTWARE` signature **1 of 3**, so it is timing-dependent there; `SDL_RENDERER` is clean **0 of 3**. A pure-virtual call is a virtual dispatch from a base destructor, so this is native teardown order rather than anything managed. | Nothing, and nothing should: the process is ending. It matters because a core dump on every `Ctrl-C` is what a developer sees all day, and because it says the native shutdown path has an ordering bug that a longer-lived process could meet another way. | Order the renderer/platform teardown so no virtual is dispatched from a base destructor, and install a signal path that unwinds the game rather than letting static destruction race the render thread. | None available and none attempted. **A normal exit is clean on all four renderers** -- the template's 600-frame run ends with exit 0 and no crash output everywhere -- so this is specific to signal-terminated shutdown and nothing in the binding's ordinary lifetime is affected. |
...
| --- | --- | --- |
| `StorageContainer.Disposing` | nothing this binding could see -- the route was there and had stopped being silent | The row said `cna_storage_container_subscribe_disposing` "is documented as synchronous and exactly once but emitted zero callbacks in the native regression", so the managed wrapper raised the known sender itself. Upstream's own `StorageSmoke.c` now subscribes, disposes, requires **exactly one** call, disposes again and requires **still exactly one**, rejects a null callback and a null out-registration, and refuses a second unsubscribe -- and it passes on this build. So the substitute is retired and `Disposing` is a real subscription taken on the first `+=` and released after the native dispose but before the handle. Proven to be native rather than inherited: with the managed one-shot gone, suppressing the subscription makes the one-shot test fail. |

The two rows below closed the same way -- by something being measured that had only been reasoned
about.


One row left the table without upstream changing anything, because a renderer that could answer it
finally existed here. It is recorded in the same shape as the rest so that "we found out" is
visible next to "they fixed it".

| Was blocked | Closed by | Proof taken against the running artifact |
| --- | --- | --- |
| A 2D-only renderer's refusal is reported as an internal error | nothing upstream -- the row described a path the C API does not take | The premise still holds in isolation: `IGraphicsRenderer::HandleUnsupported3DCall` still throws a bare `std::runtime_error` under the default `Throw` policy, and the C API's exception barrier would still map that to `CNA_RESULT_INTERNAL`. What the row could not know, because no 2D-only renderer was buildable here, is that **the C API checks the capability before the renderer is asked**, so nothing reaches it. Measured against a `SDL_RENDERER` build, which reports `ThreeD`, `CustomEffects`, `OcclusionQuery`, `Texture3D` and thirteen more absent: vertex buffers, index buffers, their dynamic twins, occlusion queries, `Texture3D`, all three draw families, cube render-target binding and cube-face storage are **every one refused with `NOT_SUPPORTED`** and a specific message. Zero `INTERNAL`. The thirteen `ThreeD` and four `CustomEffects` sites that stated this uncertainty at the call site are now asserted branches. |
